...
<!--
	****************************************************************
    ...
    Module: Stylesheet for expanding variable, style and object.
...
<xsl:stylesheet version="3.0" 
    ...
    exclude-result-prefixes="xsl xs ahf style"
>
	<!--============================================
	...
	<!-- Original WML objects -->
	<xsl:variable name="glOrgWmlObjects">
		<xsl:document>
			<xsl:apply-templates select="document($styleDefFileUri)/*" mode="GET_WML_OBJECTS">
				<xsl:with-param name="prmXmlLang" tunnel="yes" as="xs:string?" select="()"/>
			</xsl:apply-templates>
			<xsl:for-each select="$altStyleDefFileUri">
				<xsl:if test="string(.)">
					<xsl:variable name="position" as="xs:integer" select="position()"/>
					<xsl:apply-templates select="document(.)/*" mode="GET_WML_OBJECTS">
						<xsl:with-param name="prmXmlLang" tunnel="yes" as="xs:string" select="$uniqueXmlLang[position() eq $position]"/>
					</xsl:apply-templates>
				</xsl:if>
			</xsl:for-each>
	...
	<xsl:template match="style:*" mode="GET_WML_OBJECTS">
		<xsl:apply-templates mode="#current"/>
	</xsl:template> 
	
	<xsl:template match="text()" mode="GET_WML_OBJECTS">
	</xsl:template> 
	
	<xsl:template match="style:wml-object" mode="GET_WML_OBJECTS">
		<xsl:param name="prmXmlLang" tunnel="yes" required="yes" as="xs:string?"/>
		<xsl:copy>
			<xsl:copy-of select="@*"/>
			<xsl:if test="exists($prmXmlLang)">
	...
	<xsl:template match="style:include[string(@href)]" mode="GET_WML_OBJECTS">
		<xsl:apply-templates select="document(string(@href),.)" mode="#current"/>
	</xsl:template>
	...
	<xsl:variable name="glWmlObjects" as="document-node()">
		<xsl:document>
			<xsl:apply-templates select="$glOrgWmlObjects/style:wml-object" mode="MAKE_DEFINITION_WML">
			</xsl:apply-templates>	
		</xsl:document>
	</xsl:variable>

	<!-- 
	     style:wml-object template
	     Generate element name: wml-object
	     Notes: Expand variable references in attribute and text.
	            2018-03-12 t.makita
	            Top level text() is ignored! 
	  -->
	<xsl:template match="style:wml-object" mode="MAKE_DEFINITION_WML">
		<xsl:variable name="wmlObjectElem" as="element()" select="."/>
		<xsl:element name="{local-name()}">
			<xsl:copy-of select="@*[name() = $expandedAttrs]"/>
			<xsl:apply-templates mode="#current">
				<xsl:with-param name="prmXmlLang" tunnel="yes" select="if (exists($wmlObjectElem/@xml:lang)) then string($wmlObjectElem/@xml:lang) else $defaultXmlLang"/>
				<xsl:with-param name="prmDocType" tunnel="yes" select="if (exists($wmlObjectElem/@document-type)) then string($wmlObjectElem/@document-type) else $defaultDocType"/>
				<xsl:with-param name="prmPaperSize" tunnel="yes" select="if (exists($wmlObjectElem/@paper-size)) then string($wmlObjectElem/@paper-size) else $defaultPaperSize"/>
				<xsl:with-param name="prmOutputType" tunnel="yes" select="if (exists($wmlObjectElem/@output-type)) then string($wmlObjectElem/@output-type) else $defaultOutputType"/>
			</xsl:apply-templates>
		</xsl:element>
	</xsl:template>
	
	<xsl:template match="node()" mode="MAKE_DEFINITION_WML">
		<xsl:copy>
	...
	<xsl:template match="@*" mode="MAKE_DEFINITION_WML" priority="5">
		<xsl:param name="prmXmlLang" tunnel="yes" required="yes" as="xs:string?"/>
		<xsl:param name="prmDocType" tunnel="yes" required="yes" as="xs:string?"/>
		<xsl:param name="prmPaperSize" tunnel="yes" required="yes" as="xs:string?"/>
		<xsl:param name="prmOutputType" tunnel="yes" required="yes" as="xs:string?"/>
		
		<xsl:variable name="attVal" as="xs:string" select="string(.)"/>
		<xsl:attribute name="{name()}">
			<xsl:choose>
				<xsl:when test="contains($attVal,$varRefChar)">
					<xsl:variable name="expandAttVal" as="xs:string">
						<xsl:call-template name="expandExp">
							<xsl:with-param name="prmExp" select="$attVal"/>
					...
					<xsl:value-of select="$expandAttVal"/>
				</xsl:when>
				<xsl:otherwise>
	...
	<xsl:template match="text()" mode="MAKE_DEFINITION_WML" priority="5">
		<xsl:param name="prmXmlLang" tunnel="yes" required="yes" as="xs:string?"/>
		<xsl:param name="prmDocType" tunnel="yes" required="yes" as="xs:string?"/>
		<xsl:param name="prmPaperSize" tunnel="yes" required="yes" as="xs:string?"/>
		<xsl:param name="prmOutputType" tunnel="yes" required="yes" as="xs:string?"/>
		...
		<xsl:choose>
			<xsl:when test="contains($text,$varRefChar)">
				<xsl:variable name="expandTextVal" as="xs:string">
					<xsl:call-template name="expandExp">
						<xsl:with-param name="prmExp" select="$text"/>
						<xsl:with-param name="prmXmlLang" select="$prmXmlLang"/>
						<xsl:with-param name="prmDocType" select="$prmDocType"/>
						<xsl:with-param name="prmPaperSize" select="$prmPaperSize"/>
						<xsl:with-param name="prmOutputType" select="$prmOutputType"/>
				...
				<xsl:value-of select="$expandTextVal"/>
			</xsl:when>
			<xsl:otherwise>
	...
	<xsl:template match="style:variable" mode="MAKE_DEFINITION">
		...
		<xsl:variable name="varValue" as="xs:string" select="string(.)"/>
		<xsl:element name="{local-name()}">
			<xsl:copy-of select="@*[name() = $expandedAttrs]"/>
			<xsl:choose>
	...
	<!-- 
		 ...
		 Note:  Variable temporary tree is stored in $glOrgVarDefs.
				...
				<variable name="index_H_top">( $index_K_top + $index_K_height ) + 0.7mm</variable>
				2018-03-14 t.makita
				Handle varref escape char for last phase xpath evaluation.
	  -->
		...
		<xsl:variable name="tempExpandedExp" as="xs:string*">
			...
			<xsl:analyze-string select="$prmExp" regex="{$expandExpRegX}">
				...
				<xsl:non-matching-substring>
					...
					<xsl:choose>
						<!-- Vraible reference escape char -->
						<xsl:when test="starts-with($token,concat($varRefEscapeChar,$varRefChar))">
							<xsl:sequence select="substring($token,2)"/>
						</xsl:when>
	...
	<!-- 
	     ...
	     Function: Recursively process the variable reference and return the result text.
	...
	<xsl:template name="styleDump">
		...
		<xsl:result-document 
			...
			byte-order-mark="no"
			indent="yes"
			href="{concat($pTempDirUrl,'/OrgVarDefs.xml')}">
			<root>
		...
		<xsl:result-document 
			...
			byte-order-mark="no"
			indent="yes"
			href="{concat($pTempDirUrl,'/VarDefs.xml')}">
			<root>
		...
		<xsl:result-document 
			...
			byte-order-mark="no" 
			indent="yes"
			href="{concat($pTempDirUrl,'/OrgStyleDefs.xml')}">
			<root>
		...
		<xsl:result-document 
			...
			byte-order-mark="no" 
			indent="yes"
			href="{concat($pTempDirUrl,'/StyleDefs.xml')}">
			<root>
		...
		<xsl:result-document 
			...
			byte-order-mark="no" 
			indent="yes"
			href="{concat($pTempDirUrl,'/OrgWmlObjects.xml')}">
			<root>
				<xsl:copy-of select="$glOrgWmlObjects"/>
			</root>
		</xsl:result-document>
		<xsl:result-document 
			method="xml" 
			...
			byte-order-mark="no" 
			indent="yes"
			href="{concat($pTempDirUrl,'/WmlObjects.xml')}">
			<root>
				<xsl:copy-of select="$glWmlObjects"/>
			</root>
		</xsl:result-document>
	...
	<!--
	 function:	Get normalized xml:lang 
	 param:		prmXmlLang
